...
module _ {t1 t2 t3 : Type}
  ...
  module _ where
    ...
    changeMatchSem-lem1 :
      ∀ a1 b2 →
        changeSemConst match (inj₁ a1) (inj₂ (inj₂ b2)) f df g dg
      ≡
  ...
  module _ where
    ...
    changeMatchSem-lem2 :
      ∀ b1 a2 →
        changeSemConst match (inj₂ b1) (inj₂ (inj₁ a2)) f df g dg
      ≡
...
correctDeriveConst (match {t1} {t2} {t3}) = ext³ lemma
  where
    lemma : ∀ s f g →
      ...
      (⟦ match ⟧Const ⊕ changeSemConst match) s f g
...
validDeriveConst (match {t1} {t2} {t3}) =
  ...
  where
    open TernaryValid {{chAlgt (sum t1 t2)}} {{chAlgt (t1 ⇒ t3)}} {{chAlgt (t2 ⇒ t3)}} {{chAlgt t3}} ⟦ match ⟧Const (changeSemConst match)
