...
deriveConst insert-const = absV 4 (λ s ds t dt → insert (s ⊕₍ int ₎ ds) (t ⊕₍ bag ₎ dt) ⊝ insert s t)
deriveConst union-const = absV 4 (λ s ds t dt → union ds dt)
deriveConst negate-const = absV 2 (λ t dt → negate dt)
deriveConst flatmap-const = absV 4 (λ s ds t dt → flatmap (s ⊕₍ int ⇒ bag ₎ ds) (t ⊕₍ bag ₎ dt) ⊝ flatmap s t)
